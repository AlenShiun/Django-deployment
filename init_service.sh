...
sed "s/nginx-container/${CONTAINER_PREFIX}-${DJANGO_PROJECT_NAME}-nginx-container/g" ${DJANGO_PROJECT_DIR}/docker-compose.yml.example > ${DJANGO_PROJECT_DIR}/docker-compose.yml

# Set container name of django project 
if [[ "$OSTYPE" == "linux-gnu" ]]; then
    sed -i "s/web-container/${CONTAINER_PREFIX}-${DJANGO_PROJECT_NAME}-web-container/g" ${DJANGO_PROJECT_DIR}/docker-compose.yml
elif [[ "$OSTYPE" == "darwin"* ]]; then
    sed -i '' "s/web-container/${CONTAINER_PREFIX}-${DJANGO_PROJECT_NAME}-web-container/g" ${DJANGO_PROJECT_DIR}/docker-compose.yml
fi
